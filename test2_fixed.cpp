...
#include <cmath>     // For mathematical functions like cos, sin
...
public:
    ...
    void initialize() {
        player = Ship();  // Initialize the player ship
        player.active = true;
    ...
    void fireBullet() {
        ...
        if (rapidFireActive || bullets.empty() || bullets.back().velocity.x != 0 || bullets.back().velocity.y != 0) {
            ...
            bullet.velocity = {static_cast<float>(cos(radianAngle)) * 5.0f, static_cast<float>(sin(radianAngle)) * 5.0f};
